...
describe('AuthForm extras', () => {
  ...
  it('maps Firebase error message (too many requests)', async () => {
    vi.spyOn(authMod, 'signInWithEmailAndPassword').mockRejectedValue(
      new FirebaseError('auth/too-many-requests', 'x')
    );
  ...
  it('forgot password requires email', async () => {
    ...
    const btn = screen.getByRole('button', {
      name: /forgot password|send reset link/i,
    });
    fireEvent.click(btn);
  ...
  it('redirects away if already authenticated (ready gate)', async () => {
    vi.spyOn(authMod, 'onAuthStateChanged').mockImplementation(
      (_: Auth, observer: authMod.NextOrObserver<User>) => {
